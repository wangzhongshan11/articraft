...
生成一个桌面级折叠四旋翼无人机机架，仅作为无电子件的结构与机构原型。中央机身为 120 mm 见方的轻量化壳体，四个折叠臂以 90° 圆周阵列连接；每个臂长约 95 mm，具有展开定位角和折叠收纳角。每个臂根部使用可打印铰链或 M3 销轴孔，并配独立锁止卡扣。桨盘仅作为圆形安装盘，不生成可飞行部件。要求展开状态与收纳状态均无碰撞，所有臂可单独打印。输出完整可溯源产物。
同时交付：
