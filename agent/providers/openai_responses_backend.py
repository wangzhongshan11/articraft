"""OpenAI Responses API backend for tool-calling workflows."""
...
from agent.providers.openai import (
    ...
    _payload_without_reasoning_summary,
...
class OpenAIResponsesBackend:
    ...
    async def generate_with_tools(
        ...
        response = await _async_retry(
            ...
            logger=logger,
            context=f"openai[{self._host.transport}]",
        )
    ...
    def _build_request_payload(
        ...
        *,
        ...
        reasoning: dict[str, Any] = {"effort": self._host.reasoning_effort}
        if self._host.reasoning_summary:
            reasoning["summary"] = self._host.reasoning_summary
        request_payload["reasoning"] = reasoning
        if self._host.prompt_cache_key:
            request_payload["prompt_cache_key"] = self._host.prompt_cache_key
        if self._host.prompt_cache_retention:
            request_payload["prompt_cache_retention"] = self._host.prompt_cache_retention
    ...
    async def _count_request_tokens(self, request_payload: dict[str, Any]) -> int:
        ...
        if self._host._client_is_async:
            response = await self._host._client.responses.input_tokens.count(**token_count_payload)
        else:
            response = await asyncio.to_thread(
                self._host._client.responses.input_tokens.count,
                **token_count_payload,
            )
        input_tokens = (
        ...
        *,
        ...
        async def _compact_once() -> Any:
            ...
            request_kwargs: dict[str, Any] = {
                "model": self._host.compaction_model_id,
                "input": input_items,
                "instructions": system_prompt,
            }
            if self._host.prompt_cache_key:
                request_kwargs["prompt_cache_key"] = self._host.prompt_cache_key
            if self._host._client_is_async:
                return await self._host._client.responses.compact(**request_kwargs)
            return await asyncio.to_thread(self._host._client.responses.compact, **request_kwargs)
        ...
        return await _async_retry(
            ...
            max_delay=self._host.retry_max_seconds,
            logger=logger,
            context=f"openai.compact[{self._host.transport}]",
        )
    ...
    async def _request_with_http(self, request_payload: dict[str, Any]) -> Any:
        ...
        if self._host._client_is_async:
            return await self._host._client.responses.create(**request_payload)
        return await asyncio.to_thread(self._host._client.responses.create, **request_payload)
    ...
    async def _request_with_transport(
        ...
        *,
        ...
        if self._host.transport == "websocket":
            websocket_payload = (
                incremental_request_payload
        ...
        if self._host.request_timeout_seconds and self._host.request_timeout_seconds > 0:
            return await asyncio.wait_for(
                request_coro,
                timeout=float(self._host.request_timeout_seconds),
            )
    ...
    async def _request_with_websocket(
        ...
        *,
        ...
        try:
        ...
        except OpenAIWebSocketError as exc:
            if exc.code == "websocket_connection_limit_reached":
                ...
                except OpenAIWebSocketError as retry_exc:
    ...
    async def _open_websocket_connection(self) -> Any:
        ...
        return await websockets.connect(
            self._host._responses_websocket_url,
            additional_headers={"Authorization": f"Bearer {self._host._api_key}"},
            open_timeout=self._host.websocket_open_timeout_seconds,
            max_size=None,
        )
    ...
    def _websocket_connection_is_stale(self) -> bool:
        if self._websocket_opened_at is None:
            return False
        max_age = self._host.websocket_max_connection_age_seconds
        if max_age <= 0:
            return False
    ...
    async def _receive_websocket_response(self, websocket: Any) -> Any:
        while True:
            ...
            if event_type == "error":
                raise OpenAIWebSocketError.from_event(event)
