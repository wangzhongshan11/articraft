---
title: '带 D 形轴孔的直纹炉灶旋钮'
description: 'Base SDK 示例：使用 KnobGeometry 构建带裙边、直纹握持面、雕刻指示线与 D 形轴孔的家电旋钮。'
tags:
  - sdk
  - base sdk
  - knob
...
本 Base SDK 示例是 `KnobGeometry` 的紧凑参考，堆叠了最常见的真实旋钮细节：**裙边（skirt）**、**直纹侧面握持（fluted grip）**、**雕刻指示线（engraved indicator）** 与 **D 形轴孔（D-shaft bore）**。

**适用查询关键词：** `range knob`、`appliance knob`、`fluted knob`、`KnobGeometry`、`D-shaft`、`control knob`。

**参数解读：**
- 主体直径 0.042、高度 0.024，`body_style="skirted"` 带外扩裙边。
- `KnobSkirt` 裙边直径 0.052、高度 0.006，微喇叭 `flare=0.08`。
- `KnobGrip(style="fluted", count=18, depth=0.0014)` 18 条直纹。
- `KnobIndicator(style="line", mode="engraved", angle_deg=18.0)` 雕刻指示线。
- `KnobBore(style="d_shaft", diameter=0.006, flat_depth=0.001)` D 形轴孔。

**测试：** 直径约 0.048–0.056，总高约 0.027–0.033。
