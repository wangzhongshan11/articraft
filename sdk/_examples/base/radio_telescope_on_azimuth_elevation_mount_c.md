---
title: '方位-俯仰架上的射电望远镜'
description: 'Base SDK 射电望远镜示例：大型旋车碟盘、馈源喇叭、叉臂、方位基座、俯仰耳轴、结构撑杆与天文台级支撑几何。'
tags:
  - sdk
  - base sdk
  - radio telescope
...
本 Base SDK 示例是逼真天文台射电望远镜的强参考：大碟盘、厚重方位-俯仰架、叉臂、馈源结构、基座与配重。适用于 `radio telescope`、`dish antenna`、`azimuth elevation mount`、`yoke mount`、`feed horn`、`LatheGeometry`、`TorusGeometry`、`sweep_profile_along_spline`、`tube_from_spline_points` 等查询。

**值得复制的建模模式：**

- `LatheGeometry` / `TorusGeometry` 做碟壳、边缘与后部加强环。
- `sweep_profile_along_spline(...)` 做厚重叉臂与脊梁，截面可控。
- `tube_from_spline_points(...)` 做撑杆、肋条与馈源支撑。
- 双轴关节：方位连续旋转 + 俯仰旋转限位。
- 基座、轴承座、叉架、碟背肋、馈源舱与配重箱分层结构细节。
