---
title: '带格构塔柱与运行小车的高塔起重机'
description: 'Base SDK 塔吊示例：方形格构 mast、臂与平衡臂桁架、配重、回转上部、棱柱小车、悬挂滑车组与钢缆细节。'
tags:
  - sdk
  - base sdk
  - tower crane
...
本 Base SDK 示例是逼真塔式起重机的强参考：方形格构塔柱、水平臂、带配重的平衡臂、回转上部、沿臂运行小车与悬挂滑车组。适用于 `tower crane`、`lattice mast`、`truss jib`、`counter jib`、`ballast`、`trolley`、`hook block`、`slewing`、`prismatic trolley`、`tube_from_spline_points` 等查询。

**值得复制的建模模式：**

- 辅助函数在 3D 点之间放置大量圆柱杆件，驱动格构生成。
- 可复用方形 mast 与三角形桁架构建器，用于长结构装配。
- `tube_from_spline_points(...)` 自定义吊钩网格。
- 混合关节：上部连续回转 + 小车沿臂棱柱行程。
- 驾驶室、配重块、小车轮、钢缆与吊钩悬挂等机械细节分层。
