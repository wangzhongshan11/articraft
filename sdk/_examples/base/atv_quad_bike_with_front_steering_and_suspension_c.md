---
title: '带前转向与悬架的全地形四轮摩托车'
description: 'Base SDK 关节式 ATV 示例：四轮、前转向、车把、座椅、脚踏、悬架、管架纵梁、挡泥板与轮子总成。'
tags:
  - sdk
  - base sdk
  - atv
...
本 Base SDK 示例是关节式 ATV/四轮摩托的强参考：比例可信、四轮、前转向、车把、座椅、脚踏、悬架、车架纵梁、挡泥板与轮子子装配。适用于 `atv`、`quad bike`、`four wheeler`、`front steering`、`handlebars`、`suspension`、`swingarm`、`tube frame`、`superellipse_side_loft`、`tube_from_spline_points`、`MotionLimits`、`revolute`、`continuous wheel spin` 等查询。

**值得复制的建模模式：**

- 通过辅助函数重复构建轮胎、轮辋、轮毂与可选转向节视觉件。
- `superellipse_side_loft(...)` 做车身壳、座椅与挡泥板体积。
- `tube_from_spline_points(...)` 做连续弯折纵梁、减震、摆臂与多数车架杆件。
- `wire_from_points(...)` 仅用于前控制臂类构件（直线段与肘部布局需显式表达）。
- 显式转向与车轮旋转关节，并配真实 `MotionLimits`。
- 前桥成对转向关节 + 后桥连续车轮旋转。
