---
title: '带关节琴键、旋钮与打击垫的 MIDI 键盘'
description: 'Base SDK MIDI 键盘示例：定制黑白键网格、键程关节、旋转旋钮、指垫、走带按钮、弯音/调制轮与表面贴装控制件。'
tags:
  - sdk
  - base sdk
  - midi keyboard
...
---
# 带关节琴键、旋钮与打击垫的 MIDI 键盘

本 Base SDK 示例是逼真 MIDI 键盘/紧凑控制器的强参考：雕塑感后壳、黑白钢琴键、旋钮、打击垫、走带按钮、显示屏与弯音/调制轮。适用于 `midi keyboard`、`keybed`、`place_on_surface`、`LoftGeometry`、`custom mesh`、`prismatic keys`、`revolute knobs` 等查询。

**值得复制的建模模式：**

- 自定义 `MeshGeometry` 构建锥形后壳 housing。
- 程序化白键/黑键轮廓 + `LoftGeometry` 放样键帽。
- 大量重复棱柱键关节，白键网格变体用 mesh 缓存去重。
- `place_on_surface(...)` 让旋钮、垫、按钮、轮子正确贴合斜面壳体。
- 棱柱键/垫 + 旋转旋钮/轮子的混合交互表面。
