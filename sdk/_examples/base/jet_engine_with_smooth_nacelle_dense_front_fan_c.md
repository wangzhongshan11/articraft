---
title: '光滑涵道与密集前风扇的喷气发动机'
description: 'Base SDK 涡扇示例：旋车涵道壳、放样风扇与静子叶片、径向叶片阵列、分层压气机与涡轮堆叠、旁路锯齿。'
tags:
  - sdk
  - base sdk
  - jet engine
...
本 Base SDK 示例是逼真喷气发动机/涡扇的强参考：光滑涵道、密集前风扇叶片、分层压气机与涡轮细节、旁路喷管花瓣与深尾喷锥。适用于 `jet engine`、`turbofan`、`nacelle`、`fan blades`、`compressor stack`、`turbine stack`、`spinner`、`stator vanes`、`bypass nozzle`、`chevrons`、`LatheGeometry`、`section_loft`、`repair_loft`、`radial pattern`、`boolean_difference` 等查询。

**值得复制的建模模式：**

- `LatheGeometry.from_shell_profiles(...)` 做外涵道、喷管壳与整流罩。
- `section_loft(...)` + `repair_loft(...)` 做风扇叶、静子叶、涡轮叶与锯齿花瓣。
- `rotate_z(...)` 径向复制构建密集叶环。
- 同心圆柱 `boolean_difference(...)` 做环形加强带。
- 涵道、涡轮核心、风扇总成独立零件，固定关节连接，视觉件命名清晰。
