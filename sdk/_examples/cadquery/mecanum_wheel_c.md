---
title: '麦克纳姆轮'
description: '构建一套麦克纳姆轮装配体：带法兰的轮毂、沿圆周重复的安装凸耳，以及绕轮缘倾斜布置、经放样成型的辊子。示例涵盖辊面放样、环形轮毂与法兰、辐条—凸耳重复结构，以及辊子在轮缘上的倾斜排布等关键 CadQuery 几何。'
tags:
  - cadquery
  - examples
  - mecanum
...
本示例从已生成的麦克纳姆轮模型中提炼出 CadQuery 侧的核心几何：放样成型的辊面花纹、环形轮毂与法兰、重复的辐条—安装凸耳结构，以及沿轮缘倾斜布置的辊子布局。阅读时可对照代码中的轮毂厚度/半径、辊子倾角、螺栓分布圆与 `Assembly` 分件着色，理解如何将运动学上的“斜辊”落实为可制造的实体与装配关系。
