---
title: '盲文板示例'
description: '将 Unicode 盲文点阵转为凸起半球帽点，在底板上批量布置多行盲文。'
tags:
  - cadquery
  - examples
  - 盲文
...
本示例演示如何用 CadQuery 制作**凸起盲文板**：将文本行转为盲文 Unicode，再按盲文单元几何（点间距、行距、点高等）在底板上生成圆柱并倒圆角，形成近似半球形的凸点；最后用较厚底板与凸点联合体，仅保留凸点可见部分。

**实现思路：**

1. `brailleToPoints`：按 Unicode 位掩码解析每个盲文字符的 8 点布局。
2. `make_base`：按文本行数与最大行长计算底板尺寸与厚度。
3. `make_embossed_plate`：在底板上逐点布圆、挤出、顶边 `fillet` 近似球冠，再与隐藏盒体合并。
4. 点圆柱半径由 `dot_diameter` 与 `dot_height` 反推，使球冠高度符合触觉规范。

下方代码与原文一致（含英文注释与变量名），便于直接运行与对照。
