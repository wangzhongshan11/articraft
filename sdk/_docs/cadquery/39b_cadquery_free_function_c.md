# 自由函数 API（Free Function API）

需要更自由地构造单个对象时，提供无隐藏状态的自由函数 API。可能更冗长，但选择器仍可作为方法使用，其余操作为自由函数。对象放置与阵列可通过 `moved` 的各种重载实现。

当前文档不完整，更多示例见测试。

## 教程

本节演示用自由函数 API 构造 `Shape` 对象。
...
`cap()` 与 `fill()` 定义顶底面。`cap()` 相对上下文形状尽量保持曲率连续；`fill()` 则否。
...
复制并放置得到最终复合体；注意 `moved()` 多种用法：

```python
result = s.moved((-3*r, 0, 0), (3*r, 0, 0))
```

除与特定形状严格相关的放置与选择外，操作均为自由函数。
...
支持运算符与自由函数实现布尔。布尔通常较慢，宜避免在循环中执行；可先将多个实体合成 compound 再一次 union。
...
自由函数 API 支持 `extrude()`、`loft()`、`revolve()`、`sweep()`。
...
自由函数 API 支持丰富文字创建，含平面曲线上的文字与曲面上的文字。
...
某些情况宜手工添加孔或凸台等，复杂形状上可避免布尔以提升性能。可向面增删、在面上开孔，或重建已有实体。
...
基形状更复杂时，可用考虑上下文元素的局部缝合：须两步——先显式缝合壳，再构造最终实体。
